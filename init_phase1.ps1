...
# 1. Create legacy-web and move files
Write-Host "Moving existing files to legacy-web..."
if (-not (Test-Path "legacy-web")) {
    New-Item -ItemType Directory -Name "legacy-web"
}

# Move everything except .git and legacy-web
Get-ChildItem -Force | Where-Object { $_.Name -ne "legacy-web" -and $_.Name -ne ".git" -and $_.Name -ne "init_phase1.ps1" } | Move-Item -Destination "legacy-web" -Force

# 2. Create pos-app (Vite + React + TS)
Write-Host "Initializing Vite + React app..."
npm create vite@latest pos-app -- --template react-ts

# 3. Setup Tailwind CSS in pos-app
